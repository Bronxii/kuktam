// B3.0 visible-source audit only. No extractor, parser, HTTP or timing here.
import 'dart:convert';
import 'dart:io';
...
void main(List<String> args) {
  ...
  final manifest =
      (jsonDecode(
                File(
                  'results/b3_0_sources/manifest.json',
                ).readAsStringSync().replaceFirst('\ufeff', ''),
              )
              as List)
          .cast<Map<String, dynamic>>();
  final report = <Map<String, Object?>>[];
  final backup = File('results/b3_0_sources/ground_truth_before.json');
  ...
  for (var id = 21; id <= 28; id++) {
    final sourceFile = 'results/b3_0_sources/source_$id.html';
    ...
    for (final change in ingredientChanges) {
      if (id == 28 && change['before'] == '0.5 tbsp ground coriander') {
        change['category'] = 'FORMAT_ONLY';
        change['format_only'] = true;
      }
    }
    ...
    final row = <String, Object?>{
      'id': '$id',
      'domain': Uri.parse(meta['url'] as String).host,
      'snapshot_id': 'b3_0/source_$id',
      'url': meta['url'],
  ...
  for (var id = 29; id <= 30; id++) {
    ...
    report.add({
      ...
      'domain': Uri.parse(meta['url'] as String).host,
      'visible_title': null,
      'ground_truth_title_status': 'UNRESOLVED',
      'ingredient_count_visible': null,
      'ingredient_count_ground_truth_before': id == 29 ? 11 : 7,
      'ingredient_count_ground_truth_after': id == 29 ? 11 : 7,
      'corrected_ingredient_lines': <Object>[],
      'ingredient_groups': <Object>[],
      'instruction_structure_before': {'entries': id == 29 ? 6 : 5},
      'instruction_structure_after': {
        'entries': null,
        'references': <Object>[],
      },
      'source_jsonld_differences_observed': <Object>[],
      'unresolved_issues': [
        'Direct HTML fetch HTTP 403 twice (PowerShell and curl). No trustworthy local recipe HTML snapshot. Original ground truth unchanged, not approved for accuracy scoring.',
      ],
      'snapshot_id': null,
      'evidence': {
        'blocked_response': 'results/b3_0_sources/attempt2_$id.html',
        'snapshot_type': 'HTTP 403 error body, NOT recipe snapshot',
      },
    });
  }
  final summary = {
  ...
  if (write) {
    ...
    File('results/b3_0_source_truth_audit.json').writeAsStringSync(
    ...
    for (final r in report) {
      md.writeln(
        '\n## ${r['id']}\n\nForrás: ${r['url']}\n\nCím: ${r['ground_truth_title_status']}. Hozzávalók: ${r['ingredient_status']}. Elkészítés: ${r['id'] == '29' || r['id'] == '30' ? 'UNRESOLVED' : 'GROUND_TRUTH_FIXED'}. UNRESOLVED: ${r['unresolved_issues']}.\n',
      );
      for (final d
    ...
    File(
      'results/b3_0_source_truth_audit.md',
    ).writeAsStringSync('${md.toString().trimRight()}\n');
  }
  stdout.writeln(jsonEncode(summary));
  for (final r in report) {
    stdout.writeln(
      '${r['id']}: ${jsonEncode(r['source_jsonld_differences_observed'])}',
    );
  }
}
