// Offline B3.2 only. No fetch, production parser or corpus mutation.
import 'dart:convert';
import 'dart:io';
import 'package:html/parser.dart' as html;
import 'audit_b3_0.dart' as a;
...
Map<String, Object?> measure() {
  ...
  final b1 =
      jsonDecode(File('results/b3_1_batch_21-30.json').readAsStringSync())
          as Map<String, dynamic>;
  final hashes =
      jsonDecode(File('results/b3_2_snapshot_hashes.json').readAsStringSync())
          as Map<String, dynamic>;
  final records = <Map<String, Object?>>[];
  for (final id in [21, 24, 22, 23, 25, 26, 27, 28]) {
    final before = File(
      'results/b3_0_sources/source_$id.html',
    ).readAsStringSync();
    final now = File('results/b3_1_html/source_$id.html').readAsStringSync();
  ...
  records.sort((x, y) => (x['id'] as String).compareTo(y['id'] as String));
  ...
  return {
    ...
    'summary': {
      'technical_extraction_success': {
        ...
        'total': 10,
        'percent': records.length / 10 * 100,
        'evidence': 'Frozen B3.1 results; not re-fetched',
      },
      'snapshot_stability': {
        'unchanged_recipe_content': records.length,
      ...
      'source_quality': {
        'ingredients': {
          ...
          'extra_rows': 0,
          'order_errors': 0,
          'group_heading_differences': records.fold<int>(
            0,
            (n, r) => n + (r['group_heading_differences'] as List).length,
          ),
        },
    ...
    'denominators': {
      'technical': 10,
      'source_quality_recipes': records.length,
      'extractor_recipes': records.length,
      'excluded_ids': ['29', '30'],
    },
    'access_blocked': [
      for (final id in ['29', '30'])
        {
          'id': id,
          'url': (b1['results'] as List)
              .cast<Map<String, dynamic>>()
              .singleWhere((r) => r['id'] == id)['url'],
          'http_status': 403,
          'response_bytes': (b1['results'] as List)
              .cast<Map<String, dynamic>>()
              .singleWhere((r) => r['id'] == id)['html_bytes'],
          ...
          'note':
              'ACCESS / FETCH LIMITATION. Non-2xx body not retained by B3.1. Extraction skipped: zero candidates does not prove absence of JSON-LD. Excluded from content/source/extractor/parser denominators; retained as technical failure. No retry.',
        },
    ],
    'recipes': records,
  };
...
void main() {
  ...
  File('results/b3_2_snapshot_reconciliation.json').writeAsStringSync(
    '${encoded.convert([
  ...
  md.writeln(
    '\n## Access exclusions\n\n${encoded.convert(report['access_blocked'])}\n\nDenominators: ${encoded.convert(report['denominators'])}',
  );
  File(
    'results/b3_2_content_source_quality.md',
