...
Future<void> main(List<String> args) async {
  if (args.length != 1 || args.single != '--run-batch-01-10') {
    stderr.writeln('Explicit approval required: --run-batch-01-10');
    exitCode = 64;
    return;
  }
  final directory = Directory('results');
  await directory.create(recursive: true);
  final output = File('results/p1_batch_01-10.json');
  final journal = File('results/p1_batch_01-10.jsonl');
  ...
  for (var i = 1; i <= 10; i++) {
    ...
    final url = File(
      '../../test/fixtures/web_import_corpus/batch_1_01-10/website_$id.txt',
  ...
  for (final input in inputs) {
    ...
    final record = <String, Object?>{
      ...
      'html_snapshot': captured?.body.isNotEmpty == true
          ? 'html/p1_${input.id}.html'
          : null,
    };
    // Snapshot/report I/O is outside the measured URL pipeline.
    if (captured?.body.isNotEmpty == true) {
      Directory('results/html').createSync(recursive: true);
      File(
        'results/html/p1_${input.id}.html',
  ...
  final md = StringBuffer('# P1 – batch 01–10\n\n$started\n\n');
  md.writeln(
  ...
  File('results/p1_batch_01-10.md').writeAsStringSync(md.toString());
}
