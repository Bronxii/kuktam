...
HOZZÁVALÓK:
- 1.0 db 1-1.25kg/2lb 4oz-2lb 12oz ripe  tomatoes
- 1.0 db medium onion
- 1.0 db small carrot
- 1.0 db celery stick
- 2.0 db tbsp olive oil
- [javítandó] db 2 squirts of tomato purée (about 2 tsp)
- 1.0 db a good pinch of sugar
- 2.0 db bay leaves
- [javítandó] db 1.2 litres/2 pints hot vegetable stock (made with boiling water and 4 rounded tsp bouillon powder or 2 stock cubes)

WARNINGS:
Production és POC jelzések külön; a POC jelzések kézi benchmark-besorolásból származnak, nem runtime felismerésből.
- 1: production=[missingQuantity]; POC=[UNSUPPORTED_INPUT, PARSER_LIMITATION]
- 4: production=[]; POC=[UNSUPPORTED_INPUT, SILENT_BAD_FALLBACK_RISK]
- 5: production=[]; POC=[UNSUPPORTED_INPUT, SILENT_BAD_FALLBACK_RISK]
- 6: production=[ambiguousIngredient]; POC=[UNSUPPORTED_INPUT, PARSER_LIMITATION]
- 7: production=[missingQuantity]; POC=[UNSUPPORTED_INPUT]
- 9: production=[ambiguousIngredient]; POC=[UNSUPPORTED_INPUT, PARSER_LIMITATION]
...
VERDICT:
POOR — 6 correction actions / 9 ingredient rows; 0 review-only quantities. Usable=true.
